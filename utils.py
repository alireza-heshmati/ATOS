...
def objective_f(model_outputs, one_hot_labels, targets_, device, o_f_type = 'CW', kap=0): 
    if o_f_type == 'CE':
        ...
        if targets_ == None:
            ...
            comparable_val = torch.amax((1-0.9*torch.eye(len(model_outputs[0]))[targets_.cpu()].to(device))*(
                model_outputs-1e5*torch.eye(len(model_outputs[0]))[targets_.cpu()].to(device)), dim=1)
            
        else :
            targets_val = torch.amax(torch.eye(len(model_outputs[0]))[targets_.cpu()].to(device) * model_outputs, dim=-1)
...
def create_blocks(stride = 2,len_window = 4,size_of_image = 32,number_of_channel = 3):
    ...
    num = 0
    for i in range(0,num_row_block*stride,stride):
        for j in range(0,num_row_block*stride,stride):
            B[num,:,i:i+len_window , j:j+len_window] = 1
            num += 1
    return torch.from_numpy(B).float() 

# assessing validation of model
def validation(test_X,test_y , device, net,batch = 16):
...
def attack_accuracy(x_adversarial, y, net, device, batch = None):
    if batch == None :
...
def calculate_result(adver_deltas,adv_acc,time, blocks):
    ...
    if blocks!=None :
        num_blocks = number_of_perturbated_blocks(blocks,adver_deltas)
    else :
        num_blocks = [0, 0 , 0]
    ...
    dict_ = {'norm1_mean' : np.round(norm1[0], 3),
             ...
             'num_blocks_mean' : np.round( num_blocks[0] ,3),
             'num_blocks_std' : np.round( num_blocks[1] ,3),
             'num_blocks_max' : np.round( num_blocks[2] ,3),
             'accuracy' : round(accuracy,3),
             'time': time}
    return dict_

def print_norm_and_accuracy(result_dict):
    print('    num_blocks_mean:',result_dict['num_blocks_mean'],'      num_blocks_std:',result_dict['num_blocks_std'] ,'      num_blocks_max:',result_dict['num_blocks_max']  ,'\n',
          '   norm1_mean:',result_dict['norm1_mean'] ,'      norm1_std:',result_dict['norm1_std'] ,'      norm1_max:',result_dict['norm1_max'] ,'\n',
